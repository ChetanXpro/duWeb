...
import pdfImage from "../../../public/pdflogo.png";
import svgImage from "../../../public/svg.png";
import docImage from "../../../public/docs.png";
    ...
    <Tooltip label={name} placement="top">
      ...
      >
        <div className=" flex flex-col items-center  border rounded-md w-40">
          <div>
            {imagesType.includes(name.slice(-4).toLowerCase()) ? (
              <img src={url} alt="img" />
            ) : DOCUMENTS.includes(name.slice(-4).toLowerCase()) ? (
              <img height={"86rem"} width="90rem" src={docImage} alt="pdf" />
            ) : SVG.includes(name.slice(-4).toLowerCase()) ? (
              <img height={"86rem"} width="90rem" src={svgImage} alt="pdf" />
            ) : (
          ...
          <div className="flex  mb-1 justify-between text-left w-full  h-14 pl-4  flex-col">
            <p className="text-sm capitalize font-sans text-left ">
              <Text>
                {name?.length > 12 ? `${name?.slice(0, 12)}...` : name}
              </Text>
            </p>
            <p className="text-xs w-full  font-sans">
              <a href={url}>
